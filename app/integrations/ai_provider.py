"""
AI-powered reply drafting -- Claude (Anthropic), bring-your-own-key.

Optional upgrade over kb_qa.py's rule-based reply matching: when a Claude
API key is configured (Admin tab, mirrors the Gmail App Password setup),
inbound-reply drafting is generated by Claude instead, grounded in this
app's own Knowledge Base Q&A entries and stock catalog so it stays
on-topic and doesn't invent product details. Every draft this produces
still requires human approval before send, same as the rule-based path --
